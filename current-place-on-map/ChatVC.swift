...
//
//  Created by George Pazdral (work) on 1/15/18.
...
class ChatVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate,  UINavigationControllerDelegate, UIImagePickerControllerDelegate, CLLocationManagerDelegate {
    ...
    func customization() {
        ...
        button.setImage(#imageLiteral(resourceName: "addfriendcopy"), for: UIControlState.normal)
        button.addTarget(self, action: #selector(ChatVC.profileButtonPressed), for: UIControlEvents.touchUpInside)
    ...
    @objc func profileButtonPressed() {
        ...
        if keyChain.get("uid") != nil {
            ...
            let data = ["friend": self.currentUser?.id, "name": self.currentUser?.name]
        ...
        performSegue(withIdentifier: "toFriendsz", sender: nil)
    }
